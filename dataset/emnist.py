...
class EMNIST(DS):
  ...
  def download_data(self, force=False): 
    self.compressed_file = DWL.download_from_kaggle('crawford/emnist', filename='emnist_source_files.zip', force=force)

  def extract_data(self, force=False):
    self.extracted_files = DL.extract(self.compressed_file, FORCE=force)
      
  def prepare_data(self):
    data_training_ubyte = self.extracted_files + '/emnist-byclass-train-images-idx3-ubyte' 
    labels_training_ubyte = self.extracted_files + '/emnist-byclass-train-labels-idx1-ubyte'
    data_test_ubyte = self.extracted_files + '/emnist-byclass-test-images-idx3-ubyte'
    lables_test_ubyte = self.extracted_files + '/emnist-byclass-test-labels-idx1-ubyte'
  ...
  def clean_up(self):
    DS.file_delete('emnist_source_files')
    DS.file_delete('emnist_source_files.zip')
