...
class Network(object):
  ...
  def __init__(self, num_features, num_class, fc_hidden_units, apply_dropout, dataSetTrain): # default only for MNIST
    ...
    # fix and reduced ewc_batch_size from 100 (for MNIST) to 50 for large dnns and datasets
    self.ewc_batch_size = 50
    self.ewc_batches = dataSetTrain.images.shape[0] // 50
  ...
  def fc_feedforward(self, h, biases, weights, apply_dropout):
    if apply_dropout: h = tf.nn.dropout(h, self.keep_prob_input)
    for (w, b) in list(zip(weights, biases))[:-1]:
      h = self.create_fc_layer(h, w, b)
      if apply_dropout: h = tf.nn.dropout(h, self.keep_prob_hidden)
  ...
  def unaggregated_nll(self):
    ...
    biases_per_example = [self.clone_variable_list(self.biases) for _ in range(self.ewc_batch_size)]
    weights_per_example = [self.clone_variable_list(self.weights) for _ in range(self.ewc_batch_size)]
  ...
  def create_fc_layer(input, w, b, apply_relu=True):
    with tf.name_scope('fc_layer'):
      ...
      if apply_relu: output = tf.nn.relu(output)
    return output

  @staticmethod
  def create_variable(shape, name, c=None, sigma=None, trainable=True):
    if sigma: initial = tf.truncated_normal(shape, stddev=sigma, name=name)
    else: initial = tf.constant(c if c else 0.0, shape=shape, name=name)
    return tf.Variable(initial, trainable=trainable)

  @staticmethod
  def clone_variable_list(variable_list):
    ''' transport variables from gpu to cpu? '''
